...
async def create_item(
    ...
    result = await session.execute(
        ...
        RETURNING id, name, type, point_cost, asset_url, exclusive_season_id, exclusive_top_n, active
    """),
        {"name": body.name, "type": body.type, "cost": body.point_cost, "url": body.asset_url},
    )
    await session.commit()
    return _store_item_dict(result.mappings().one())
...
async def update_item(
    ...
    if body.point_cost is not None:
        updates["point_cost"] = body.point_cost
    if body.asset_url is not None:
        updates["asset_url"] = body.asset_url
    ...
    result = await session.execute(
        text(f"""
        UPDATE league_store_items SET {set_clause}
        WHERE id = :id
        RETURNING id, name, type, point_cost, asset_url, exclusive_season_id, exclusive_top_n, active
    """),
...
async def get_balance(
    ...
    balance = (await session.execute(text("SELECT COALESCE(SUM(delta), 0) FROM league_points_ledger WHERE engineer_id = :uid"), {"uid": user["user_id"]})).scalar()
...
async def list_items(
    ...
    Developers see only active items. Admin Store Editor passes
    ?include_inactive=true to also see deactivated items (so they can
    reactivate them).
    """
    if include_inactive:
...
async def purchase_item(
    ...
    item = (await session.execute(text("SELECT id, point_cost, active, exclusive_season_id, exclusive_top_n FROM league_store_items WHERE id = :id"), {"id": str(item_id)})).mappings().one_or_none()
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
    if not item["active"]:
    ...
    already_owned = (await session.execute(text("SELECT COUNT(*) FROM league_purchases WHERE engineer_id = :uid AND item_id = :iid"), {"uid": user["user_id"], "iid": str(item_id)})).scalar()
    if already_owned:
        raise HTTPException(status_code=409, detail="Item already owned")

    await session.execute(text("INSERT INTO league_purchases (engineer_id, item_id, points_spent) VALUES (:uid, :iid, :pts)"), {"uid": user["user_id"], "iid": str(item_id), "pts": item["point_cost"]})
