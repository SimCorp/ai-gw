...
CHALLENGES = [
        ...
        "hidden_test_suite": [
            {
                ...
                "expected_output": "Increases the cache TTL from 60 to 3600 seconds. Risk: stale data may persist much longer; downstream systems that assume sub-minute freshness will break. No tests are added.",
                "weight": 1.0,
            },
            {
                "input": "diff --git a/auth/session.py b/auth/session.py\n-    HMAC_KEY = settings.SECRET\n+    HMAC_KEY = settings.SESSION_SIGNING_KEY",
                "expected_output": "Splits session signing from the generic SECRET into a dedicated SESSION_SIGNING_KEY. Risk: deploys that haven't set the new env var will fail to sign sessions and lock users out. No tests are added.",
                "weight": 1.5,
            },
            {
                "input": "diff --git a/util/retry.py b/util/retry.py\n+def retry(fn, attempts=3):\n+    for i in range(attempts):\n+        try: return fn()\n+        except: pass",
                "expected_output": "Adds a bare retry helper that swallows all exceptions across three attempts. Risk: silent failure mode — original exceptions are discarded so callers cannot diagnose failures. No tests are added for the new helper.",
                "weight": 1.0,
            },
        ],
    },
    {
        ...
        "hidden_test_suite": [
            {
                "input": "psycopg2.OperationalError: FATAL: remaining connection slots are reserved for non-replication superuser connections",
                "expected_output": '{"category": "infrastructure", "confidence": 0.95, "rationale": "Postgres connection pool exhausted — database tier capacity issue, not application logic."}',
                "weight": 1.0,
            },
            {
                "input": "AttributeError: 'NoneType' object has no attribute 'split'\n  File 'parser.py', line 87, in tokenize\n    parts = self.cache.get(key).split(',')",
                "expected_output": '{"category": "application_bug", "confidence": 0.9, "rationale": "Cache returned None and the code path did not guard against the miss — a programming error in our own module."}',
                "weight": 1.2,
            },
            {
                "input": "boto3.exceptions.S3UploadFailedError: An error occurred (AccessDenied) when calling the PutObject operation: Access Denied",
                "expected_output": '{"category": "infrastructure", "confidence": 0.8, "rationale": "IAM policy denies the upload — configuration/permissions issue in the cloud environment."}',
                "weight": 1.0,
            },
        ],
    },
    {
        ...
        "hidden_test_suite": [
            {
                "input": "SELECT u.name, (SELECT COUNT(*) FROM posts p WHERE p.user_id = u.id) AS post_count FROM users u;\n-- Plan: nested loop, COUNT subquery per user row.\n-- Indexes: posts(user_id)",
                "expected_output": "SELECT u.name, COALESCE(p.post_count, 0) AS post_count FROM users u LEFT JOIN (SELECT user_id, COUNT(*) AS post_count FROM posts GROUP BY user_id) p ON p.user_id = u.id;",
                "weight": 1.5,
            },
            {
                "input": "SELECT * FROM events WHERE created_at::date = '2026-01-15';\n-- Plan: Seq Scan, function on indexed column.\n-- Indexes: events(created_at)",
                "expected_output": "SELECT * FROM events WHERE created_at >= '2026-01-15' AND created_at < '2026-01-16';",
                "weight": 1.0,
            },
        ],
    },
        ...
        "hidden_test_suite": [
            {
                ...
                "expected_output": '{"issues": [{"severity": "high", "line": 3, "message": "Untrusted URL fetched without scheme or host validation — SSRF risk."}, {"severity": "medium", "line": 3, "message": "No timeout — call can hang indefinitely."}]}',
                "weight": 1.5,
            },
            {
                "input": "def store_password(user, pwd):\n    user.password = pwd\n    user.save()",
                "expected_output": '{"issues": [{"severity": "high", "line": 2, "message": "Passwords must be hashed (e.g. argon2/bcrypt) before storage — never persist plaintext."}]}',
                "weight": 1.5,
            },
            {
                "input": "def divide(a, b):\n    return a / b",
                "expected_output": '{"issues": [{"severity": "medium", "line": 2, "message": "No guard against b == 0 — will raise ZeroDivisionError at runtime."}]}',
                "weight": 1.0,
            },
        ],
    },
    {
        ...
        "hidden_test_suite": [
            {
                "input": "Alert: db_connection_pool_exhausted\nMetric: postgres.connections.idle = 0 / max 100\nService: orders-api\nTime: 2026-04-12 02:11 UTC",
                "expected_output": '{"actions": ["Identify which queries are holding connections via pg_stat_activity", "Check for long-running transactions or deadlocks", "Restart the highest-CPU orders-api pod to release connections if no transaction is critical"], "escalate_if": "pool stays at 0 idle for 10+ minutes or active queries exceed 30s"}',
                "weight": 1.0,
            },
            {
                "input": "Alert: error_rate_spike\nMetric: app.http.5xx.rate = 12% (threshold 1%)\nService: checkout-api\nTime: 2026-04-12 19:45 UTC",
                "expected_output": '{"actions": ["Group recent 5xx by error type to identify dominant exception", "Diff most recent deploy commit against last known-good version", "Roll back checkout-api to previous deployment if errors trace to a code change"], "escalate_if": "error rate stays above 5% after rollback or affects payment confirmations"}',
                "weight": 1.2,
            },
            {
                "input": "Alert: disk_usage_critical\nMetric: host.disk.used = 96% (threshold 85%)\nService: log-aggregator-1\nTime: 2026-04-12 06:30 UTC",
                "expected_output": '{"actions": ["Run du -h /var/log | sort -h to find largest contributors", "Check whether log rotation cron is failing", "Expire oldest log files older than retention policy"], "escalate_if": "disk reaches 99% before mitigation or rotation cannot be restored"}',
                "weight": 1.0,
            },
        ],
    },
    {
        ...
        "hidden_test_suite": [
            {
                "input": "OLD: GET /accounts/{id} returns 200 with {balance: number}\nNEW: GET /accounts/{id} returns 200 with {balance: string}",
                "expected_output": '{"added": [], "removed": [], "breaking": ["GET /accounts/{id} balance type changed from number to string — clients parsing it as a number will break"], "semantic": []}',
                "weight": 1.5,
            },
            {
                "input": "OLD: DELETE /sessions/{id} returns 204\nNEW: (endpoint removed)",
                "expected_output": '{"added": [], "removed": ["DELETE /sessions/{id}"], "breaking": ["DELETE /sessions/{id} no longer exists — clients calling it will receive 404"], "semantic": []}',
                "weight": 1.0,
            },
            {
                "input": "OLD: GET /search?q=string (returns up to 100 results)\nNEW: GET /search?q=string (returns up to 25 results, pagination required for more)",
                "expected_output": '{"added": [], "removed": [], "breaking": [], "semantic": ["GET /search default page size reduced from 100 to 25 — clients relying on the larger page may miss results"]}',
                "weight": 1.2,
            },
        ],
    },
    ...
    },
    {
        ...
        "hidden_test_suite": [
            {
                "input": 'def serialize(obj: dict) -> str:\n    """Round-trippable JSON serialization. Example: {\'a\':1} -> \'{"a":1}\' and deserialize(serialize(x)) == x"""',
