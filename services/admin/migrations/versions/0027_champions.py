...
def upgrade():
    op.create_table(
        ...
        sa.Column(
            "nominated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()
        ),
        sa.Column(
            "nominated_by",
            sa.dialects.postgresql.UUID(as_uuid=True),
    ...
    op.create_table(
        ...
        sa.Column(
            "champion_id",
            sa.dialects.postgresql.UUID(as_uuid=True),
            sa.ForeignKey("champions.developer_id", ondelete="CASCADE"),
            nullable=False,
        ),
        ...
        sa.Column(
            "auto_metadata", sa.dialects.postgresql.JSONB(), nullable=False, server_default="{}"
        ),
    )
    op.create_index("ix_champion_contributions_champion", "champion_contributions", ["champion_id"])
    op.create_index(
        "ix_champion_contributions_submitted_at", "champion_contributions", ["submitted_at"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "contribution_id",
            sa.dialects.postgresql.UUID(as_uuid=True),
            sa.ForeignKey("champion_contributions.id", ondelete="CASCADE"),
            primary_key=True,
        ),
        sa.Column(
            "created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()
