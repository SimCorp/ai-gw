...
@router.put("/{team_id}")
async def update_team(team_id: UUID, body: TeamCreate, session: AsyncSession = Depends(get_session)):
    team = await session.get(Team, team_id)
    if not team:
        raise HTTPException(status_code=404, detail="Team not found")
    team.name = body.name
...
async def delete_team(team_id: UUID, session: AsyncSession = Depends(get_session)):
    team = await session.get(Team, team_id)
    if not team:
        raise HTTPException(status_code=404, detail="Team not found")
...
async def create_project(team_id: UUID, body: ProjectCreate, session: AsyncSession = Depends(get_session)):
