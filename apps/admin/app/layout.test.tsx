...
describe('RootLayout (admin) — Rybbit tracker', () => {
  beforeEach(() => {
    vi.resetModules();
  });
  ...
  it('injects tracker script when all three env vars are set', async () => {
    ...
    vi.stubEnv('NEXT_PUBLIC_RYBBIT_SITE_ID', 'site-123');
  ...
  it('omits tracker script when RYBBIT_ENABLED is not "true"', async () => {
    vi.stubEnv('NEXT_PUBLIC_RYBBIT_ENABLED', 'false');
    vi.stubEnv('NEXT_PUBLIC_RYBBIT_URL', 'https://rybbit.aigw.scdom.net');
    vi.stubEnv('NEXT_PUBLIC_RYBBIT_SITE_ID', 'site-123');
    const { default: RootLayout } = await import('./layout');
    const { queryByTestId } = render(<RootLayout><div /></RootLayout>);
